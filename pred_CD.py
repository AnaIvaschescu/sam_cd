...
class PredOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--chkpt_path', required=False, default=os.path.join(working_path, DATA_NAME, 'LevirCD_SAM_CD_e42_OA99.24_F86.73_IoU79.30.pth') )
        ...
        with torch.no_grad():
              if imgA.shape[0]>opt.crop_size[0] or imgA.shape[1]>opt.crop_size[1]:
                  ...
                  for idx in range(crop_num):
                    ...
                    #   tensorA = transF.to_tensor(cropA).unsqueeze(0).to(torch.device('cpu')).float()
                    #   tensorB = transF.to_tensor(cropB).unsqueeze(0).to(torch.device('cpu')).float()
